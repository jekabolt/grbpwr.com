...
import { cartProductToOrderItemInsert } from "@/lib/api-adapters/order";
import {
  clearIdempotencyKey,
...
export function getValidationErrorToastKey(error: unknown): string {
  ...
  if (
    PAYMENT_UNAVAILABLE_MESSAGES.some((m) => message.toLowerCase().includes(m))
  ) {
...
async function callValidateOrderItemsInsert(params: {
...
export async function validateCartItems({
  ...
  const items: common_OrderItemInsert[] = products.map(
    cartProductToOrderItemInsert,
  );
  ...
  const requestParams = {
    ...
    shipmentCarrierId:
      shipmentCarrierId !== undefined ? Number(shipmentCarrierId) : undefined,
    country,
    paymentMethod,
    currency,
  };
  ...
  const tryValidate = async (
    ...
    const hasCheckoutParams =
      country && currency && isCardPaymentMethod(paymentMethod);

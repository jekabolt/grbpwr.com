...
export function parseClientIdFromGaCookieString(
  cookieValue: string,
): string | undefined {
...
// GA4 forbids PII in events. Two leaks are scrubbed centrally here so no single
// emitter can reintroduce them: (1) the email lives in the URL on
// /order/<uuid>/<base64email> and /unsubscribe/<email>, which GA4 otherwise
// auto-collects as `page_location`; (2) emitters that forward `page_path`/URLs.
const PII_URL_KEYS = [
  "page_path",
  "page_location",
  "page_url",
...
// Event params that must never carry raw PII regardless of who set them.
const PII_STRIP_KEYS = ["email", "user_email", "phone", "phone_number"];

/** Redact an email (raw, %40-encoded, or the base64 order segment) from a path. */
export function sanitizeAnalyticsPath(value: string): string {
  if (!value) return value;
  return value
    .replace(/(\/order\/[^/]+\/)[^/?#]+/gi, "$1[redacted]")
    .replace(/(\/unsubscribe\/)[^/?#]+/gi, "$1[redacted]")
    .replace(/([?&](?:email|e)=)[^&#]*/gi, "$1[redacted]")
    .replace(/[^/?&=#]*(?:@|%40)[^/?&=#]*/gi, "[redacted]");
}

/** Redact PII from a full URL, preserving origin so page_location stays useful. */
export function sanitizeAnalyticsUrl(value: string): string {
  ...
  try {
    ...
    return `${u.origin}${sanitizeAnalyticsPath(`${u.pathname}${u.search}`)}${u.hash}`;
  } catch {
    return sanitizeAnalyticsPath(value);
  }
}

/** Strip raw-PII keys and redact email from URL-bearing params before send. */
function scrubEventParams(
  params: Record<string, unknown>,
): Record<string, unknown> {
  const out: Record<string, unknown> = {};
  for (const [key, value] of Object.entries(params)) {
    if (PII_STRIP_KEYS.includes(key)) continue;
    if (typeof value === "string" && PII_URL_KEYS.includes(key)) {
      out[key] =
        key === "page_path"
...
/** Current full URL with any email redacted — overrides GA4's auto page_location. */
function safePageLocation(): string {
  if (typeof window === "undefined") return "";
  return sanitizeAnalyticsUrl(window.location.href);
}
